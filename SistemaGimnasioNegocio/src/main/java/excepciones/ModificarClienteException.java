/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package excepciones;

/**
 *
 * @author 52644
 */
public class ModificarClienteException extends Exception{

    public ModificarClienteException() {
    }

    public ModificarClienteException(String message) {
        super(message);
    }

    public ModificarClienteException(String message, Throwable cause) {
        super(message, cause);
    }

    public ModificarClienteException(Throwable cause) {
        super(cause);
    }

    public ModificarClienteException(String message, Throwable cause, boolean enableSuppression, boolean writableStackTrace) {
        super(message, cause, enableSuppression, writableStackTrace);
    }
    
}
